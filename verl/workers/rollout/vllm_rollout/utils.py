...
class vLLMOmniColocateWorkerExtension(_OmniWorkerBase):
    ...
    def _update_weights(self, weights: list[tuple[str, torch.Tensor]], peft_config: dict, base_sync_done: bool):
        if peft_config and base_sync_done:
            # Actor trains the full model (Qwen3OmniMoeForConditionalGeneration) where
            # LoRA params have paths like "...thinker.model.layers.0...". The rollout
            # engine loads the thinker-only model which expects "...model.layers.0...".
            # Strip the "thinker.model." prefix so names match the thinker-only model.
            weights = {
